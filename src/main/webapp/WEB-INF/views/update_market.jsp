...
$(document).ready(function(){
	$("#btn_Updateok").click(function(){
		if($("#Update_market_name").val()==""){
			alert("정보를 제대로 입력해주세요");
			document.getElementById("Update_market_name").focus();
		}else if($("#Update_market_tel").val()==""){
			alert("정보를 제대로 입력해주세요");
			document.getElementById("Update_market_tel").focus();
		}else if($("#Update_market_address").val()==""){
			alert("정보를 제대로 입력해주세요");
			document.getElementById("Update_market_address").focus();
		}else if($("#Update_market_content").val()==""){
			alert("정보를 제대로 입력해주세요");
			document.getElementById("Update_market_content").focus();
			...
			var time1 = $("#market_time1 option:selected").val();
	   		...
	   		var update_market_Img = $("#Update_market_img").val();
			$('#update_market_Img').val(update_market_Img);
	   		
	   		//alert(ftype);
			document.getElementById('market_updatedata').submit();
		}
	})
...
<body>
    <form id="market_updatedata" action='update_marketdata' method="post" enctype="multipart/form-data">
    <div class="container">
    	<%
		...
		if(session.getAttribute("master")!=null){ Mdto = (Login_MasterDto)session.getAttribute("master"); %>
			<input type="hidden" id="Update_market_master" name="Update_market_master" value="<%=Mdto.getMaster_market()%>"/>
			<input type="hidden" id="market_no" name="market_no" value="<%=Mdto.getMaster_no()%>"/>
		<%}%>
        <div class="row">
            <div class="col text-center" style="margin-top: 50px;">
            	<img id="Img" style="width: 150px;height: 150px;margin-top: 10px;">
            	<input class="d-xl-flex" type="file" style="margin-left: 700px;" name="Update_market_img" id="Update_market_img" onchange="LoadImg(this);">
            	<input type="hidden" id="update_market_Img" name="update_market_Img" value=""/>
            </div>
        </div>
        <div class="row">
            <div class="col text-center" style="margin-top: 35px;">
            <input type="text" style="width: 500px;height: 43px;" placeholder="가게이름" name="market_name" id="Update_market_name"></div>
        </div>
        <div class="row">
            <div class="col text-center" style="margin-top: 35px;">
            <input type="text" id="market_tel" style="width: 500px;height: 43px;" placeholder="가게 전화번호" name="market_tel" id="Update_market_tel"></div>
        </div>
        <div class="row">
            <div class="col text-center" style="margin-top: 35px;">
            <input type="text" id="market_address" style="width: 500px;height: 43px;" placeholder="가게 주소" name="market_address" id="Update_market_address"></div>
        </div>
        <div class="row">
            <div class="col text-center" style="margin-top: 35px;">
            <input type="text" id="market_content" style="width: 500px;height: 43px;" placeholder="가게 설명" name="market_content" id="Update_market_content"></div>
        </div>
        <div class="row">
            <div class="col text-center" style="margin-top: 35px;">
            <p>영업시간</p>
    ...
    <div class="row">
        <div class="col text-center" style="margin-top: 30px;">
        	<button class="btn btn-primary" id="btn_Updateok" type="button" style="margin-right: 115px;background-color: #18c2d9;">수정</button>
        	<button class="btn btn-primary" id="btn_Updatecancle" type="button" style="background-color: #18c2d9;">취소</button>
        </div>
    </div>
    </div>
    </form>
